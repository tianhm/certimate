...
const BizDeployNodeConfigFieldsProviderCMCCCloudVLB = () => {
  ...
  return (
    <>
      <Form.Item
        name={[parentNamePath, "poolId"]}
        initialValue={initialValues.poolId}
        label={t("workflow_node.deploy.form.cmcccloud_vlb_pool_id.label")}
        rules={[formRule]}
        tooltip={<span dangerouslySetInnerHTML={{ __html: t("workflow_node.deploy.form.cmcccloud_vlb_pool_id.tooltip") }}></span>}
      >
        <Input placeholder={t("workflow_node.deploy.form.cmcccloud_vlb_pool_id.placeholder")} />
      </Form.Item>
      ...
      >
        <Select
          options={[DEPLOY_TARGET_LOADBALANCER, DEPLOY_TARGET_LISTENER].map((s) => ({
            label: t(`workflow_node.deploy.form.cmcccloud_vlb_deploy_target.option.${s}.label`),
            value: s,
          }))}
      ...
      <Show when={fieldResourceType === DEPLOY_TARGET_LOADBALANCER}>
        <Form.Item
          ...
          label={t("workflow_node.deploy.form.cmcccloud_vlb_loadbalancer_id.label")}
          rules={[formRule]}
          tooltip={<span dangerouslySetInnerHTML={{ __html: t("workflow_node.deploy.form.cmcccloud_vlb_loadbalancer_id.tooltip") }}></span>}
        >
          <Input placeholder={t("workflow_node.deploy.form.cmcccloud_vlb_loadbalancer_id.placeholder")} />
        </Form.Item>
      </Show>
      ...
      <Show when={fieldResourceType === DEPLOY_TARGET_LISTENER}>
        <Form.Item
          ...
          label={t("workflow_node.deploy.form.cmcccloud_vlb_listener_id.label")}
          rules={[formRule]}
          tooltip={<span dangerouslySetInnerHTML={{ __html: t("workflow_node.deploy.form.cmcccloud_vlb_listener_id.tooltip") }}></span>}
        >
          <Input placeholder={t("workflow_node.deploy.form.cmcccloud_vlb_listener_id.placeholder")} />
        </Form.Item>
      </Show>
      ...
      <Show when={fieldResourceType === DEPLOY_TARGET_LOADBALANCER || fieldResourceType === DEPLOY_TARGET_LISTENER}>
        <Form.Item
          ...
          label={t("workflow_node.deploy.form.cmcccloud_vlb_snidomain.label")}
          extra={t("workflow_node.deploy.form.cmcccloud_vlb_snidomain.help")}
          rules={[formRule]}
        >
          <Input allowClear placeholder={t("workflow_node.deploy.form.cmcccloud_vlb_snidomain.placeholder")} />
        </Form.Item>
      </Show>
    </>
  );
...
const _default = Object.assign(BizDeployNodeConfigFieldsProviderCMCCCloudVLB, {
  getInitialValues,
  getSchema,
});
