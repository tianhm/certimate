...
import (
	...
	certmgrimpl "github.com/certimate-go/certimate/pkg/core/certmgr/providers/byteplus-medialive"
...
func (d *Deployer) getAllDomains(ctx context.Context) ([]string, error) {
	...
	// 查询域名列表
	// REF: https://docs.byteplus.com/en/docs/byteplus-media-live/docs-listdomaindetail-2023
	...
	for {
		...
		listDomainDetailReq := &bplive.ListDomainDetailBody{
			DomainStatusList: bp.Int32Slice([]int32{0}),
			PageNum:          int32(listDomainDetailPageNum),
			PageSize:         int32(listDomainDetailPageSize),
		}
		listDomainDetailResp, err := d.sdkClient.ListDomainDetail(listDomainDetailReq)
		d.logger.Debug("sdk request 'medialive.ListDomainDetail'", slog.Any("request", listDomainDetailReq), slog.Any("response", listDomainDetailResp))
		if err != nil {
			return nil, fmt.Errorf("failed to execute sdk request 'medialive.ListDomainDetail': %w", err)
		}
...
func (d *Deployer) updateDomainCertificate(ctx context.Context, domain string, cloudCertId string) error {
	// 绑定证书
	// REF: https://docs.byteplus.com/en/docs/byteplus-media-live/docs-bindcert-2023
	bindCertReq := &bplive.BindCertBody{
		ChainID: cloudCertId,
		Domain:  domain,
		HTTPS:   bp.Bool(true),
	}
	bindCertResp, err := d.sdkClient.BindCert(bindCertReq)
	d.logger.Debug("sdk request 'medialive.BindCert'", slog.Any("request", bindCertReq), slog.Any("response", bindCertResp))
	if err != nil {
		return err
	}
