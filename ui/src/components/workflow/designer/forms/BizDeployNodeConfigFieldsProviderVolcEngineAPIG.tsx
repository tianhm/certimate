...
const BizDeployNodeConfigFieldsProviderVolcEngineAPIG = () => {
  ...
  return (
    <>
      <Form.Item
        ...
        label={t("workflow_node.deploy.form.volcengine_apig_region.label")}
        rules={[formRule]}
        tooltip={<span dangerouslySetInnerHTML={{ __html: t("workflow_node.deploy.form.volcengine_apig_region.tooltip") }}></span>}
      >
        <Input placeholder={t("workflow_node.deploy.form.volcengine_apig_region.placeholder")} />
      </Form.Item>
      ...
      <Show when={fieldDomainMatchPattern !== DOMAIN_MATCH_PATTERN_CERTSAN}>
        <Form.Item
          ...
          label={t("workflow_node.deploy.form.volcengine_apig_domain.label")}
          rules={[formRule]}
        >
          <Input placeholder={t("workflow_node.deploy.form.volcengine_apig_domain.placeholder")} />
        </Form.Item>
      </Show>
    </>
  );
...
const getSchema = ({ i18n = getI18n() }: { i18n?: ReturnType<typeof getI18n> }) => {
  ...
  return z
    .object({
      region: z.string().nonempty(t("workflow_node.deploy.form.volcengine_apig_region.placeholder")),
...
const _default = Object.assign(BizDeployNodeConfigFieldsProviderVolcEngineAPIG, {
  getInitialValues,
  getSchema,
});
