...
const BizDeployNodeConfigFieldsProviderBytePlusCLB = () => {
  ...
  return (
    <>
      <Form.Item
        ...
        label={t("workflow_node.deploy.form.byteplus_clb_region.label")}
        rules={[formRule]}
        tooltip={<span dangerouslySetInnerHTML={{ __html: t("workflow_node.deploy.form.byteplus_clb_region.tooltip") }}></span>}
      >
        <Input placeholder={t("workflow_node.deploy.form.byteplus_clb_region.placeholder")} />
      </Form.Item>
      ...
      >
        <Select
          options={[DEPLOY_TARGET_LOADBALANCER, DEPLOY_TARGET_LISTENER].map((s) => ({
            value: s,
            label: t(`workflow_node.deploy.form.byteplus_clb_deploy_target.option.${s}.label`),
          }))}
      ...
      <Show when={fieldResourceType === DEPLOY_TARGET_LOADBALANCER}>
        <Form.Item
          ...
          label={t("workflow_node.deploy.form.byteplus_clb_loadbalancer_id.label")}
          rules={[formRule]}
          tooltip={<span dangerouslySetInnerHTML={{ __html: t("workflow_node.deploy.form.byteplus_clb_loadbalancer_id.tooltip") }}></span>}
        >
          <Input placeholder={t("workflow_node.deploy.form.byteplus_clb_loadbalancer_id.placeholder")} />
        </Form.Item>
      </Show>
      ...
      <Show when={fieldResourceType === DEPLOY_TARGET_LISTENER}>
        <Form.Item
          ...
          label={t("workflow_node.deploy.form.byteplus_clb_listener_id.label")}
          rules={[formRule]}
          tooltip={<span dangerouslySetInnerHTML={{ __html: t("workflow_node.deploy.form.byteplus_clb_listener_id.tooltip") }}></span>}
        >
          <Input placeholder={t("workflow_node.deploy.form.byteplus_clb_listener_id.placeholder")} />
        </Form.Item>
      </Show>
    </>
  );
...
const _default = Object.assign(BizDeployNodeConfigFieldsProviderBytePlusCLB, {
  getInitialValues,
  getSchema,
});
